...
function sincronizarCatalogo() {
  ...
  rows.forEach(function (row) {
    ...
    var midia = extrairHyperlink_(row['midia__hyperlink'] || row['midia'] || '')
  ...
  // Deduplica por sku_base
  var vistos = {}
  var deduplicados = []
  for (var i = registros.length - 1; i >= 0; i--) {
    var chave = registros[i].sku_base
  ...
  if (duplicatas > 0) log_('  Duplicatas removidas: ' + duplicatas)

  upsertSupabase_('catalogo_produtos', deduplicados, 'sku_base')
  log_('Produtos sincronizados: ' + deduplicados.length)
}
...
function sincronizarVariacoes() {
  ...
  rows.forEach(function (row) {
    ...
    var ativo = ativoRaw === '' || ativoRaw === 'SIM' || ativoRaw === 'TRUE' || ativoRaw === '1'
  ...
  // Filtra variações cujo sku_base não existe no catalogo_produtos (evita FK violation)
  var skuBasesValidos = buscarSkuBasesValidos_()
  var semCatalogo = []
  var validos = deduplicados.filter(function (r) {
    if (skuBasesValidos[r.sku_base]) return true
    semCatalogo.push(r.sku_base)
    return false
  })

  if (semCatalogo.length > 0) {
    log_('  AVISO: ' + semCatalogo.length + ' variacao(oes) ignorada(s) por sku_base ausente no catalogo: ' + [...new Set(semCatalogo)].slice(0, 10).join(', '))
  }

  if (validos.length === 0) {
    log_('Nenhuma variação válida para sincronizar.')
    return
  }

  upsertSupabase_('variacoes_sku', validos, 'sku')
  log_('Variações sincronizadas: ' + validos.length)
}
...
function upsertSupabase_(tabela, registros, chave) {
  ...
  var url = (props['SUPABASE_URL'] || '').replace(/\/$/, '') + '/rest/v1/' + tabela + '?on_conflict=' + chave
...
function buscarSkuBasesValidos_() {
  var props = PropertiesService.getScriptProperties().getProperties()
  var url = (props['SUPABASE_URL'] || '').replace(/\/$/, '') + '/rest/v1/catalogo_produtos?select=sku_base&limit=10000'
  var key = props['SUPABASE_KEY'] || ''
  ...
  var produtos = JSON.parse(response.getContentText()) || []
  var mapa = {}
  produtos.forEach(function (p) {
    if (p.sku_base) mapa[p.sku_base] = true
  })
  return mapa
}
...
function buscarMapaLojas_() {
  ...
  lojas.forEach(function (loja) {
    if (loja.nome) mapa[loja.nome] = loja.id
  })
  return mapa
}
...
// ------------------------------------------------------------
// LEITURA DE PLANILHA (independente do Code.gs)
// ------------------------------------------------------------
